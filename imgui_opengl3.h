...
// Desktop OpenGL: attempt to detect default GL loader based on available header
// files. If auto-detection fails or doesn't select the same GL loader file as
// used by your application, you are likely to get a crash in
// ImGui_ImplOpenGL3_Init(). You can explicitly select a loader by using
// '#define IMGUI_IMPL_OPENGL_LOADER_XXX' in imconfig.h or compiler
// command-line.
...
//  2019-10-25: OpenGL: Using a combination of GL define and runtime GL version
//  to decide whether to use glDrawElementsBaseVertex(). Fix building with
//  pre-3.2 GL loaders. 2019-09-22: OpenGL: Detect default GL loader using
//  __has_include compiler facility. 2019-09-16: OpenGL: Tweak initialization
//  code to allow application calling ImGui_ImplOpenGL3_CreateFontsTexture()
//  before the first NewFrame() call. 2019-05-29: OpenGL: Desktop GL only: Added
//  support for large mesh (64K+ vertices), enable
//  ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: OpenGL: Added
//  support for special ImDrawCallback_ResetRenderState callback to reset render
//  state. 2019-03-29: OpenGL: Not calling glBindBuffer more than necessary in
//  the render loop. 2019-03-15: OpenGL: Added a dummy GL call + comments in
//  ImGui_ImplOpenGL3_Init() to detect uninitialized GL function loaders early.
//  2019-03-03: OpenGL: Fix support for ES 2.0 (WebGL 1.0).
//  2019-02-20: OpenGL: Fix for OSX not supporting OpenGL 4.5, we don't try to
//  read GL_CLIP_ORIGIN even if defined by the headers/loader. 2019-02-11:
...
//  2019-02-01: OpenGL: Using GLSL 410 shaders for any version over 410 (e.g.
//  430, 450). 2018-11-30: Misc: Setting up io.BackendRendererName so it can be
//  displayed in the About Window. 2018-11-13: OpenGL: Support for GL 4.5's
//  glClipControl(GL_UPPER_LEFT) / GL_CLIP_ORIGIN. 2018-08-29: OpenGL: Added
//  support for more OpenGL loaders: glew and glad, with comments indicative
//  that any loader can be used. 2018-08-09: OpenGL: Default to OpenGL ES 3 on
//  iOS and Android. GLSL version default to "#version 300 ES". 2018-07-30:
//  OpenGL: Support for GLSL 300 ES and 410 core. Fixes for Emscripten
//  compilation. 2018-07-10: OpenGL: Support for more GLSL versions (based on
//  the GLSL version string). Added error output when shaders fail to
//  compile/link. 2018-06-08: Misc: Extracted imgui_impl_opengl3.cpp/.h away
//  from the old combined GLFW/SDL+OpenGL3 examples. 2018-06-08: OpenGL: Use
//  draw_data->DisplayPos and draw_data->DisplaySize to setup projection matrix
//  and clipping rectangle. 2018-05-25: OpenGL: Removed unnecessary
//  backup/restore of GL_ELEMENT_ARRAY_BUFFER_BINDING since this is part of the
//  VAO state. 2018-05-14: OpenGL: Making the call to glBindSampler() optional
//  so 3.2 context won't fail if the function is a NULL pointer. 2018-03-06:
//  OpenGL: Added const char* glsl_version parameter to ImGui_ImplOpenGL3_Init()
//  so user can override the GLSL version e.g. "#version 150". 2018-02-23:
//  OpenGL: Create the VAO in the render function so the setup can more easily
//  be used with multiple shared GL context. 2018-02-16: Misc: Obsoleted the
...
bool ImGui_ImplOpenGL3_Init(const char* glsl_version) {
    ...
    if (g_GlVersion >= 3200)
        ...
        // Store GLSL version string so we can refer to it later in case we
        // recreate shaders. Note: GLSL version is NOT the same as GL version.
    ...
    // Dummy construct to make it easily visible in the IDE and debugger which
    // GL loader has been selected. The code actually never uses the 'gl_loader'
    // variable! It is only here so you can read it! If auto-detection fails or
    // doesn't select the same GL loader file as used by your application, you
    // are likely to get a crash below. You can explicitly select a loader by
    // using '#define IMGUI_IMPL_OPENGL_LOADER_XXX' in imconfig.h or compiler
    // command-line.
...
// up/restoring every OpenGL state explicitly, in order to be able to run within
// any OpenGL engine that doesn't do so.
